...
class LocalDataSource(private val db: OneTillDb) {
    ...
    suspend fun getAllProducts(): List<Product> = withContext(Dispatchers.Default) {
        queries.selectAllProducts().executeAsList().map { assembleProduct(it) }
    }

    suspend fun getProductById(id: Long): Product? = withContext(Dispatchers.Default) {
        queries.selectProductById(id).executeAsOneOrNull()?.let { assembleProduct(it) }
    }

    suspend fun getProductByBarcode(barcode: String): Product? = withContext(Dispatchers.Default) {
    ...
    suspend fun getProductCount(): Long = withContext(Dispatchers.Default) {
        queries.selectProductCount().executeAsOne()
    }

    suspend fun saveProduct(product: Product) = withContext(Dispatchers.Default) {
        db.transaction {
    ...
    suspend fun saveProducts(products: List<Product>) = withContext(Dispatchers.Default) {
        db.transaction {
            for (product in products) {
                // Inline save to stay in the same transaction
                queries.insertOrReplaceProduct(
                    id = product.id,
                    name = product.name,
                    sku = product.sku,
    ...
    suspend fun deleteAllProducts() = withContext(Dispatchers.Default) {
    ...
    suspend fun saveOrder(order: Order): Long = withContext(Dispatchers.Default) {
    ...
    suspend fun getPendingSyncOrders(): List<Order> = withContext(Dispatchers.Default) {
        queries.selectPendingSyncOrders().executeAsList().map { assembleOrder(it) }
    }

    suspend fun getRecentOrders(limit: Int): List<Order> = withContext(Dispatchers.Default) {
    ...
    suspend fun updateOrderStatus(localId: Long, status: OrderStatus) = withContext(Dispatchers.Default) {
    ...
    suspend fun saveTaxRates(rates: List<TaxRate>) = withContext(Dispatchers.Default) {
        db.transaction {
    ...
    suspend fun getAllTaxRates(): List<TaxRate> = withContext(Dispatchers.Default) {
    ...
    suspend fun getLastSyncedAt(entityType: String): Instant? = withContext(Dispatchers.Default) {
